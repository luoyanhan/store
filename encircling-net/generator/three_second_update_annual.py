...
def creditcode_producer(creditcode_queue, config_name):
    ...
    while True:
        ...
        #                         )).order_by(Baseinfo.id.asc()).limit(ADD_TO_QUEUE_EVERYTIME).all()
        #
        #     for each in creditcodes:
        #         keyno = each[1]
        #         result = AnnualCheYear.get_list_by_keyno(keyno, 1, 100)
        #         years = [each.ancheyear for each in result['data']]
        #         if each[2]:
        #             creditcode_queue.put([each[2], years], block=True, timeout=None)
        #         else:
        #             creditcode_queue.put([each[3], years], block=True, timeout=None)
        if start_id:
            start_id = int(start_id.decode())
            creditcodes = session.query(Baseinfo.id, Baseinfo.keyno, Baseinfo.creditcode, Baseinfo.firm). \
                filter(and_(Baseinfo.id > start_id,
                            Baseinfo.creditcode.like('91%')
                            )).order_by(Baseinfo.id.asc()).limit(ADD_TO_QUEUE_EVERYTIME).all()
        else:
            creditcodes = session.query(Baseinfo.id, Baseinfo.keyno, Baseinfo.creditcode, Baseinfo.firm). \
